...
objects have the same identity, then they are necessarily equal.
...
the variables `b` and `c`, on the other hand, refer to the same object:
...
You can check that `b` and `c` refer to the same object by evaluating
...
to `not (x is y)`. 
If you need to use `is` and `not` as distinct operators, 
use parentheses to avoid ambiguity, as in `x is (not y)`. 
