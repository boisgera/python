...
description: Duck typing, inheritance, protocols, and standard library modules
...
The function `copy_file` below reads the content of a file object and writes it to another:
...
after the quote attributed to James Whitcomb Riley

> When I see a bird that walks like a duck and swims like a duck and 
> quacks like a duck, I call that bird a duck. 🦆

Note that for the moment, the constraints that argument `input` of the function
`copy_file` must satisfy is only a contract (moral) between the designer of
...
to refer to this contract (or of **concept** or **interface**).

At this stage, the Python interpreter is **not** informed of this contract and
does nothing particular to ensure that the mutual commitment is respected.
It will therefore be up to the developer of the function to document this protocol
and to the user to read and respect it.

## Static Verification

There are tools that allow (partially) formalizing the contracts on which your programs
depend, for example [mypy](http://mypy-lang.org/).
...
In other words, pure paths -- instances of `PurePath` -- allow us to designate files
but without accessing the file system properly. Instances of `Path` -- which derives from `PurePath` -- allow it.
...
For example, on my machine (Linux), I can designate the root of the file system
...
The `random` function of the module will generate floating-point numbers uniformly
...
Multiple functions are provided for generating pseudo-random numbers following various
...
Studying the source file [random.py](https://github.com/python/cpython/blob/3.10/Lib/random.py)
...
The default method `random` generates random integers between $0$ and $2^{53} - 1$
(the probability of each integer being identical), then divides the result by
$2^{53}$. The downside of this approach: `random()` returns a value that is always a
multiple of $2^{-53}$. The floating-point number $2^{-1074}$, for example, which is the
smallest strictly positive floating-point number, has no chance of being produced.
...
by [the documentation of the module `random`](https://docs.python.org/3/library/random.html#recipes) by defining a class
derived from `Random` that overrides the method `random`
...
multiples of $2^{-53}$ (there is nevertheless more than a one in two chance
that it is the case).
...
The other probability distributions using the method `random` as source of
random values, we don't need to reimplement anything else to benefit hugely
...
It verifies that the examples in your documentation conform to the actual behavior of your code.
...
as erroneous with the symbol 🐛 (🇺🇸 **bug** or 🇫🇷 **bogue**).
This will serve as a reminder!

To do this, we will derive from the class [`OutputChecker`] of `doctest` and
override its method `check_output` to signal that any test whose result contains a bug
symbol should be considered as passed. Then we will insert the resulting class
in place of the `OutputChecker` class of `doctest`, to change the behavior of the
module.
