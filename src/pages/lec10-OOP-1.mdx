...
description: Objects, classes, methods, and properties in Python
...
Before discovering how to use objects, it is good to be convinced that
this skill will be very useful in Python, because:

> Everything that can be designated by a variable is an object!

Technically:

> It is an **instance** of the **type** `object`.

<Admonition type="note" title="Instance?" open={true}>

The term "instance" should be interpreted in its Anglo-Saxon sense:

> 🇺🇸 an individual illustrative of a category

(cf. the [Merriam-Webster dictionary entry for "instance"](https://www.merriam-webster.com/dictionary/instance))

We can therefore consider that a type is a category of objects;
its instances are then the objects that belong to it.
...
But yes, because both can be true simultaneously!
We have already found that `42` is indeed an object:
```python
>>> isinstance(42, object)
True
```
Let's also verify that it is an integer:
```python
>>> isinstance(42, int)
True
```
So the two properties are not contradictory.
More precisely, the type of `42` is integer, not object:
```python
>>> type(42) == int
True
```
...
Although it may be less intuitive, functions, types, or modules are also objects:
...
The type `complex` in Python represents complex numbers.
It provides a good example of the interactions we can have with objects.
...
It is good to know this syntax because it is the one Python will use to represent
complex numbers:
...
Not all objects have such a **literal** notation. But there is an alternative method for all objects:
...
**attributes**. All complex numbers thus have the attributes `real` and `imag`:
...
should be `z.real = ...`. However, this assignment fails because complex
numbers were designed as non-modifiable.
...
A complex number has an attribute `conjugate` whose nature is a bit special.
It is a **method**:
...
Methods behave like functions: they are callable.
...
Our objective in this section will be to create a type `Complex` which will be a
simplified version of the built-in type `complex`. To do this, we will define a new
**class** of objects; to create a minimal class, without specific functionality,
...
which would allow us to take an empty complex number and add the desired attributes to it:
...
The convention when such a method is defined is to call the first argument of the method
`self`, which will always designate an instance of the class considered
...
building a suitable string representation of instances.
...
We then have a representation compatible with the literal notation of built-in complex numbers
...
Not all attributes of an object are necessarily meant to be **public**; we may want
**private** data, for internal use, only usable by the methods specific to an object.
The convention in Python is to prefix the name of such attributes with a single underscore.
...
For example, we can make it so that when we want to set the value of the real or imaginary
...
constructor to benefit from this additional security. Of course, since we have made the real
and imaginary parts private, we will need to provide read access functions so that external
users of complex numbers can use them. Internally, we need to adapt methods to use private
