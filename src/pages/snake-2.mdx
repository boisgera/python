...
This makes the code more readable and easier to modify: screen dimensions,
frame rate, colors, and direction vectors all get descriptive names.
...
Separate the constants into a dedicated `constants.py` module and import them
into the main `snake.py` file. This reduces clutter in the main file and
makes the constants reusable across multiple files.
...
Implement a function `display` that draws a list or set of colored pixels.
...
Then extend the `display` function so that:
  ...
  - when the second argument is a single pixel and not a list or set 
    of pixels, it should apply the color to this pixel.
