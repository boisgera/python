...
It is the references to objects that are stored in lists, not
the objects themselves; by modifying an element in a list, you also modify
...
The notation `{}` does not define an empty set, but an empty dictionary[^dict].
...
[^dict]: Dictionaries existed in Python well before sets were introduced.
They therefore occupied the `{}` notation first, and sets had to accommodate it afterward.
